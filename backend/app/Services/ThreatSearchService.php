...
class ThreatSearchService
{
    ...
    /**
     * Search for threat intelligence data about any observable type.
     *
     * Queries OpenCTI for observable data, relationships, indicators,
     * sightings, and notes. Falls back to ip-api.com for geolocation
     * when the observable is an IP address. Results are cached for 15 minutes.
     *
     * @param  string  $query  The observable value to search (IP, domain, URL, email, hostname, hash)
    ...
    private function executeSearch(string $query): array
    {
        ...
        try {
            $observable = $this->queryObservable($query);
            ...
            // Query format error — continue with found=false
        }
    ...
    /**
     * Query OpenCTI for a STIX Cyber Observable matching the query.
     *
     * Uses a single value filter (no entity_type constraint) so any
     * observable type can be matched. Hash inputs are detected and
     * routed to hash-specific filter keys.
     */
    private function queryObservable(string $query): ?array
    {
        $graphql = <<<'GRAPHQL'
    ...
    /**
     * Detect if the query is a hash and return the appropriate GraphQL filter key.
     *
     * @return string|null  The hash filter key (e.g. 'hashes.MD5') or null if not a hash
     */
    ...
    private function buildResponse(
        ...
        return [
            'query' => $query,
            'detected_type' => $observable['entity_type'] ?? null,
            'found' => $found,
